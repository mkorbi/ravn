...
| D12 | Embedding-Modell (Phase 2) | **Qwen3-Embedding-0.6B (1024 dim, multilingual)** | `Qwen/Qwen3-Embedding-0.6B` via fastembed-rs. ~1.2 GB Modell, MTEB-Top-Tier Q2 2026, explizit multilingual (DE/EN). `messages_vec` + `skills_vec` haben `vec0(embedding float[1024])`. Auf macOS via ONNX-Runtime ohne CUDA. |
