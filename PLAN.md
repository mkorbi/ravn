...
- [x] **6.2** Reward-Funktionen für verifizierbare Skills (Tests grün, Git-Commit, File-Diff matched). → `ravn_eval::reward`: `Verifier`-Trait + `RewardOutcome`; `TestsPass` (Command exit 0), `GitCommitted` (HEAD ≠ base_rev), `FileMatches` (Exact/Contains). `score()` = Mean der Rewards, `passed` = alle bestanden. `score_and_record()` schreibt ein `react.reward`-Event (`record_reward` in persistence); `export_jsonl` hängt den Episode-Reward an den Terminal-Schritt der Trajektorie. Tests: alle drei Verifier (temp repo/file/cmd) + Composite + Export-Join.
- [x] **6.3** `bin/curator`: nightly Job sucht häufige Action-Sequenzen, abstrahiert zu SKILL.md-Kandidaten. → `ravn_eval::curator`: `mine()` zählt contiguous Tool-n-grams (Länge 2–4) über distinkte Trajektorien (optional reward-gefiltert), ab `min_support` → `SkillCandidate` (Rang nach Support/Länge); `render_skill_md()` erzeugt valides SKILL.md (`name`/`description`/`allowed_tools`). Bin `curator` (im `eval`-Crate) schreibt Kandidaten nach `~/.ravn/skill-candidates/<name>/SKILL.md` (separat von aktiven Skills; nur Vorschläge → Verifikation 6.4). `trajectory::load()` für strukturierten Zugriff extrahiert. Tests: Mining/Support, Reward-Filter, Render + Roundtrip-Parse durch `ravn_skills::load_skill`. Als Heartbeat (4.10) schedulebar.
- [x] **6.4** Skill-Synthesis-Verification: Dry-Run auf historischen Tasks, Merge nur bei Pass-Rate-Verbesserung. → `ravn_eval::synthesis`: `decide(baseline, candidate, min_improvement)` (Promote iff `candidate ≥ baseline + margin`); `promote()` verschiebt einen Kandidaten **atomar** (`fs::rename`) von `skill-candidates/<name>` in den Live-Skills-Ordner und verweigert das Überschreiben bestehender Skills. `PassRateMeasurer`-Trait + `verify_and_promote()` (misst baseline vs. mit-Kandidat — LLM-gebunden, daher injiziert). Bin `verify-candidate` promotet anhand extern gemessener Pass-Rates. Tests: decide-Fälle, promote (move/clobber/missing), Orchestrator mit Mock-Measurer. (Atomic Rollback + Git-Versionierung folgt in 6.5.)
- [x] **6.5** Skill-Repo: Git-versioned, atomic Rollback bei Regression. → `ravn_eval::skillrepo::SkillRepo` macht den Live-Skills-Ordner zum Git-Repo (`ensure_init` legt Identity + Basis-Commit an, `head`, `commit_all`, `rollback_to` = `reset --hard` + `clean`). `promote_committed()` = `synthesis::promote` + ein Commit pro Promotion mit `{rev, parent}` → Rollback per `rollback_to(parent)` macht genau diese Promotion atomar rückgängig (andere bleiben). `verify-candidate --git` committet die Promotion und zeigt den Rollback-Befehl. Tests: promote→commit→rollback entfernt den Skill + stellt HEAD wieder her; zweite Promotion ist separat revertierbar.
...
- [x] **6.9** Constitutional Self-Auditing: User-definierte Verfassung (Markdown) + nightly Auditor-Agent gegen letzte 100 Sessions, Findings → MEMORY.md. → `ravn_eval::audit`: `Constitution::load` (`~/.ravn/constitution.md`), `load_transcripts(db, limit)` rendert die letzten N Sessions (Text + Tool-Marker, gecappt), `Auditor` (LLM gegen die Verfassung → JSON-`Finding[]`, analog `judge`), `write_findings` hängt eine datierte `## Audit findings`-Section an `memory.md`. Bin `auditor` (`--limit`/`--dry-run`, key-gated). Tests: Transcript-Load aus In-Memory-DB, Findings-Parse (Array/leer/fence), Memory-Append (+ No-op bei leer). Als Heartbeat (4.10) schedulebar.
